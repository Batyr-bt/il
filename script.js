const INVITE = {
  ...
  when: {
    ...
    isoLocal: "2026-03-08T17:00",
    displayFallback: "8.03.2026 | 17:00",
  },
  ...
  message:
    "С собой бери прекрасное настроение!\nОтказ не принимается.",
};
...
function init() {
  ...
  // Подпись внизу — компактно и аккуратно /html www.com
  setText("fromText", INVITE.fromName);
  setText("whenText", formatWhen(INVITE.when.isoLocal, INVITE.when.displayFallback));
  ...
  // Закрытие по Esc (полезно на десктопе)
  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape" && app?.dataset.state === "open") close();
  });

  // Удобно закрывать свайпом вниз? Не добавляю жесты — только простая кнопка.
}
